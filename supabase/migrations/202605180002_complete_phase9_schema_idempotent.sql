...
begin
  ...
  if not exists (select 1 from pg_trigger where tgname = 'set_video_analyses_updated_at') then
    create trigger set_video_analyses_updated_at before update on public.video_analyses
    for each row execute function public.set_updated_at();
  end if;

  if not exists (select 1 from pg_trigger where tgname = 'set_analysis_steps_updated_at') then
    create trigger set_analysis_steps_updated_at before update on public.analysis_steps
    for each row execute function public.set_updated_at();
  end if;
  ...
  if not exists (select 1 from pg_trigger where tgname = 'set_vocabulary_items_updated_at') then
    create trigger set_vocabulary_items_updated_at before update on public.vocabulary_items
    for each row execute function public.set_updated_at();
  end if;
end;
$$;
...
create index if not exists video_analyses_video_id_idx on public.video_analyses(video_id);
create index if not exists analysis_steps_analysis_id_idx on public.analysis_steps(analysis_id);
create index if not exists chapters_video_id_order_idx on public.chapters(video_id, order_index);
create index if not exists timeline_items_video_id_order_idx on public.timeline_items(video_id, order_index);
create index if not exists subtitle_segments_video_id_order_idx on public.subtitle_segments(video_id, order_index);
create index if not exists notes_user_video_created_idx on public.notes(user_id, video_id, created_at desc);
create index if not exists chat_threads_user_video_idx on public.chat_threads(user_id, video_id);
create index if not exists chat_messages_thread_created_idx on public.chat_messages(thread_id, created_at);
create index if not exists vocabulary_items_user_text_idx on public.vocabulary_items(user_id, text);
create index if not exists vocabulary_sources_video_idx on public.vocabulary_sources(video_id);
create index if not exists vocabulary_sources_item_idx on public.vocabulary_sources(vocabulary_item_id);
...
drop policy if exists "Authenticated users can read video analyses" on public.video_analyses;
create policy "Authenticated users can read video analyses"
on public.video_analyses for select to authenticated using (true);

drop policy if exists "Authenticated users can insert video analyses" on public.video_analyses;
...
drop policy if exists "Authenticated users can update video analyses" on public.video_analyses;
...
drop policy if exists "Authenticated users can read analysis steps" on public.analysis_steps;
create policy "Authenticated users can read analysis steps"
on public.analysis_steps for select to authenticated using (true);

drop policy if exists "Authenticated users can insert analysis steps" on public.analysis_steps;
...
drop policy if exists "Authenticated users can update analysis steps" on public.analysis_steps;
...
drop policy if exists "Authenticated users can read timeline items" on public.timeline_items;
create policy "Authenticated users can read timeline items"
on public.timeline_items for select to authenticated using (true);

drop policy if exists "Authenticated users can insert timeline items" on public.timeline_items;
...
drop policy if exists "Authenticated users can read subtitle segments" on public.subtitle_segments;
...
drop policy if exists "Authenticated users can insert subtitle segments" on public.subtitle_segments;
...
drop policy if exists "Users can manage own chat threads" on public.chat_threads;
...
drop policy if exists "Users can manage own vocabulary items" on public.vocabulary_items;
create policy "Users can manage own vocabulary items"
on public.vocabulary_items for all to authenticated
using (user_id = auth.uid())
with check (user_id = auth.uid());

drop policy if exists "Users can manage own vocabulary sources" on public.vocabulary_sources;
